...
export class FineController {
    constructor(private readonly _fineService: FineService) {}


    /**
     * Registra una multa en la blockBlockchainFineStatuschain y sube la evidencia a IPFS.
     */
    async registerFine (req: Request, res: Response, next: NextFunction) {
        try {
            ...
            const result = await this._fineService.registerFine(req.file, req.body);
     ...
     */
    async updateFineStatus (req: Request, res: Response, next: NextFunction) {
        try {
            ...
            const result = await this._fineService.updateFineStatus(parseInt(fineId, 10), newState, reason);
    ...
    async getFine (req: Request, res: Response, next: NextFunction) {
        try {
            ...
            const fineDetails = await this._fineService.getFineDetails(numericFineId);
    ...
    async getFines (req: Request, res: Response, next: NextFunction) {
        try {
            ...
            const fines = await this._fineService.getAllFines(page, pageSize);
            res.status(200).json({
     ...
     */
    async getFineEvidence (req: Request, res: Response, next: NextFunction) {
        try {
            ...
            const stream = await this._fineService.getFineEvidence(evidenceCID);
     ...
     */
    async verifyBlockchainIntegrity (req: Request, res: Response, next: NextFunction) {
        try {
            const { fineId } = req.params;
            if (!fineId) {
                return res.status(400).json({ message: "Fine ID is required." });
            ...
            const integrityResult = await this._fineService.verifyBlockchainIntegrity(parseInt(fineId, 10));
     ...
     */
    async getFineFromSIMIT (req: Request, res: Response, next: NextFunction) {
        try {
            ...
            const simitData = await this._fineService.getFineFromSIMIT(plateNumber);
     ...
     */
    async linkFineToSIMIT (req: Request, res: Response, next: NextFunction) {
        try {
            const { fineId } = req.params;
            const { simitId } = req.body;
            const transactionHash = await this._fineService.linkFineToSIMIT(parseInt(fineId, 10), simitId);
     ...
     */
    async getFinesByPlate (req: Request, res: Response) {
        try {
            ...
            const finesDetails = await this._fineService.getFinesByPlate(plateNumber);
     ...
     */
    async getFineStatusHistory (req: Request, res: Response, next: NextFunction) {
        try {
            const { fineId } = req.params;
            ...
            const statusHistory = await this._fineService.getFineStatusHistory(parseInt(fineId, 10));
     ...
     */
    async getRecentFinesHistory (req: Request, res: Response, next: NextFunction) {
        try {
            const recentHistory = await this._fineService.getRecentFinesHistory();
