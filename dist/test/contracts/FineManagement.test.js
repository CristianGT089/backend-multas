...
describe("FineManagement Contract", function () {
    ...
    describe("Operator Management", function () {
        ...
        it("Non-owner should not be able to add or remove an operator", async function () {
            await expect(fineManagement.connect(user1).addOperator(addrs[0].address)).to.be.revertedWith("Ownable: caller is not the owner");
            await expect(fineManagement.connect(user1).removeOperator(operator1.address)).to.be.revertedWith("Ownable: caller is not the owner");
        });
    });
    describe("Fine Registration", function () {
        it("Operator should be able to register a fine", async function () {
            const tx = await fineManagement.connect(operator1).registerFine(PLATE_NUMBER, EVIDENCE_CID, LOCATION, INFRACTION_TYPE, COST, OWNER_IDENTIFIER, EXTERNAL_ID);
        ...
        it("Non-operator should not be able to register a fine", async function () {
            await expect(fineManagement.connect(user1).registerFine(PLATE_NUMBER, EVIDENCE_CID, LOCATION, INFRACTION_TYPE, COST, OWNER_IDENTIFIER, EXTERNAL_ID)).to.be.revertedWith("Not an operator");
        });
        it("Should increment fine ID for subsequent fines", async function () {
            await fineManagement.connect(operator1).registerFine("PLT001", "CID001", "LOC1", "TYPE1", ethers.parseUnits("10", "ether"), "OWN1", "EXT1");
            const tx2 = await fineManagement.connect(operator1).registerFine("PLT002", "CID002", "LOC2", "TYPE2", ethers.parseUnits("20", "ether"), "OWN2", "EXT2");
    ...
    describe("Fine Status Update", function () {
        ...
        it("Operator should be able to update fine status", async function () {
            await expect(fineManagement.connect(operator1).updateFineStatus(fineId, 1, "Payment received"))
        ...
        it("Non-operator should not be able to update fine status", async function () {
            await expect(fineManagement.connect(user1).updateFineStatus(fineId, 1, "Payment received")).to.be.revertedWith("Not an operator");
        });
        it("Should not allow updating to the same status", async function () {
            await expect(fineManagement.connect(operator1).updateFineStatus(fineId, 0, "Already pending")).to.be.revertedWith("Fine: Status is already the same");
        });
        it("Should revert for invalid fine ID", async function () {
            await expect(fineManagement.connect(operator1).updateFineStatus(999, 1, "Invalid ID")).to.be.revertedWith("Fine: Invalid ID");
        });
    });
});
//# sourceMappingURL=FineManagement.test.js.map