...
export class BlockchainRepository {
    ...
    constructor() {
        console.log("Configuración del servicio blockchain:", {
            ...
            ownerAddress: new ethers.Wallet(blockchainConfig.privateKey).address
        });
        ...
        this.contract = new ethers.Contract(blockchainConfig.contractAddress, FineManagementArtifact.abi, this.wallet);
    }
    static getInstance() {
    ...
    async registerFine(plateNumber, evidenceCID, location, infractionType, cost, ownerIdentifier, externalSystemId = "") {
        try {
            ...
            const tx = await this.contract.registerFine(plateNumber, evidenceCID, location, infractionType, ethers.parseUnits(cost.toString(), 0), ownerIdentifier, externalSystemId);
    ...
    async getFinesDetails(page = 1, pageSize = 10) {
        try {
            ...
            return fines
                .filter((fine) => fine && fine.id) // Asegurarse de que la multa es válida
                .map((fine) => ({
                id: fine.id.toString(),
                plateNumber: fine.plateNumber,
                evidenceCID: fine.evidenceCID,
    ...
    async getFinesByPlate(plateNumber) {
        try {
            ...
            console.log("Fines encontradas por placa:", fineIds.map((id) => id.toString()));
            return fineIds.map((id) => id.toString());
        }
        catch (error) {
    ...
    async linkFineToSIMIT(fineId, simitId) {
        try {
            const tx = await this.contract.updateFineStatus(fineId, 0, `Linked to SIMIT: ${simitId}`);
            const receipt = await tx.wait();
            return receipt.hash;
        }
        catch (error) {
    ...
    async verifyBlockchainIntegrity(fineId) {
        try {
            ...
            if (statusHistory.length > 0) {
                ...
                const sortedHistory = [...statusHistory].sort((a, b) => Number(b.lastUpdatedTimestamp) - Number(a.lastUpdatedTimestamp));
                ...
                if (Number(lastStatus.newState) !== Number(fine.currentState)) {
                    verificationDetails.push(`El estado actual (${fine.currentState}) no coincide con el último estado del historial (${lastStatus.newState}). ` +
     ...
     */
    async getFineStatusHistory(fineId, page = 1, pageSize = 10) {
        try {
            ...
            const decodedResult = contractInterface.decodeFunctionResult("getFineStatusHistory", result);
